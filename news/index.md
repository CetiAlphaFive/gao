...
- Fixed daily data updates: new reports are now discovered through GAO’s
  official RSS feed after a change on gao.gov broke the previous
  approach. If the feed is ever unavailable, the update fails with a
  clear error instead of quietly adding nothing.
...
- [`gao_links()`](https://cetialphafive.github.io/gao/reference/gao_links.md)
  now returns additional analysis-ready columns: `issuing_division` and
  `product_type` (decoded from report IDs), temporal covariates
  (`pub_month`, `pub_dow`, `pub_fiscal_year`, `fiscal_quarter`,
...
- Much cleaner requester fields: stray text no longer appears in
  `requester_committees` / `requester_members`; multi-word, hyphenated,
  and suffixed member names (Van Hollen, Ros-Lehtinen, Wasserman
  Schultz) now resolve; House/Senate members who share a surname no
  longer collide; and independents are labeled `"Other"` with majority
  status resolved via the party they caucus with.
...
- New
  ...
  downloads the latest dataset (refreshed daily) without any extra
  software;
  [`gao_links()`](https://cetialphafive.github.io/gao/reference/gao_links.md)
  then uses it automatically.
- [`auto_download()`](https://cetialphafive.github.io/gao/reference/auto_download.md)
  offers to check for updated data in interactive sessions.

## gao 0.4.0

- New
  [`extract_text()`](https://cetialphafive.github.io/gao/reference/extract_text.md)
  extracts text from downloaded PDFs (requires the `pdftools` package).
...
- **Breaking:**
  [`gao_links()`](https://cetialphafive.github.io/gao/reference/gao_links.md)
  now returns a data.frame with full report metadata instead of a
  character vector of URLs.
- Fiscal-year filtering now uses publication dates rather than patterns
  in report IDs, fixing missing years for roughly a third of older
  reports.
...
- New
  [`auto_download()`](https://cetialphafive.github.io/gao/reference/auto_download.md):
  load the dataset, filter by year, and download reports in one call,
  with interactive prompts and a confirmation guard against accidental
  mass downloads.
...
- Initial release: bundled dataset of ~55,000 GAO report URLs
  (1921–present),
  [`gao_links()`](https://cetialphafive.github.io/gao/reference/gao_links.md)
  for browsing, and batch download of reports as PDF or HTML.
